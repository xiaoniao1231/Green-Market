...
/* ---------- 文件消息：图片 / 视频 / 音频在聊天里直接预览，其它文件只提供下载 ----------
   类型判定依据文件名扩展名（后端 content 列存的就是原始文件名）；文件名没有
   扩展名时退化为用 OSS 地址里的扩展名判断。 */
...
/* ---------- 预览窗口（图片放大 / 视频播放共用） ----------
   退出方式：右上角的 × 按钮，或按 Esc —— 点窗口以外的区域不会退出。 */
...
<template>
  <div>
    ...
    <!-- 预览窗口（图片放大 / 视频播放）：右上角 × 或 Esc 关闭；
         点窗口以外的区域不会关闭 -->
    <div v-if="state.preview" class="media-lightbox">
      <button class="ml-close" type="button" title="关闭（Esc）" aria-label="关闭" @click="closePreview">✕</button>
      <div class="ml-body">
